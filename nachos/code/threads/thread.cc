...
Thread::Thread(char* threadName)
{
    ...
    stackTop = NULL;
    stack = NULL;
    status = static_cast<ThreadStatus>(JUST_CREATED);
...
void 
Thread::Fork(Process* process, VoidFunctionPtr func, void *arg)
{
    ...
    process->scheduler->ReadyToRun(this);	// ReadyToRun assumes that interrupts
					// are disabled!
...
Thread::Yield ()
{
    ...
    nextThread = kernel->currentProcess->scheduler->FindNextToRun();
    if (nextThread != NULL) {
	kernel->currentProcess->scheduler->ReadyToRun(this);
	kernel->currentProcess->scheduler->Run(nextThread, FALSE);
    }
...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;
    while ((nextThread = kernel->currentProcess->scheduler->FindNextToRun()) == NULL)
	kernel->interrupt->Idle();	// no one to run, wait for an interrupt
    
    // returns when it's time for us to run
    kernel->currentProcess->scheduler->Run(nextThread, finishing); 
}
...
Thread::SelfTest()
{
    ...
    Process *t = new Process("forked thread",4);

    t->currentThread->Fork(t, (VoidFunctionPtr) SimpleThread, (void *) 1);
    kernel->currentProcess->Yield();
    SimpleThread(0);
}
